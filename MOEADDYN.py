...
def get_ref(population_size, dimension):
    fix = int(population_size * 0.6)
    dyn = population_size - fix
    ref = [[0 for _ in range(2)] for _ in range(population_size)]
    fix_ratio = 1 / fix
    dyn_ratio = 1 / (4 * dyn)
    for i in range(0, fix):
        ref[i][0] = fix_ratio * i + 1/dimension
    for i in range(fix, population_size):
        ref[i][0] = dyn_ratio * (i - fix) + 1/dimension
    return ref
...
def MOEADDYN(file_name, dimension, population_size, max_time):
    ...
    ref_points = get_ref(population_size, dimension)
    ...
    while time.time() - begin_time < max_time:
        ...
        if iteration % 10 == 0 and iteration > 0 and flag:
            if interval == 0:
                ...
                for i in range(fix, population_size):
                    population[i] = fix_solution(population[i], acur, int(ref_points[i][0] * dimension))
                obj, z = evaluate_solution(population, data)
                fitness = evaluate_fitness(population, obj, ref_points)
            else:
                pre_erate = 2
                ...
                for i in range(population_size):
                    if ref_points[i][0] < frate1 and obj[i][1] < pre_erate:
                        pre_erate = obj[i][1]
                    if frate1 <= ref_points[i][0] < frate2 and obj[i][1] < cur_erate:
                        cur_erate = obj[i][1]
            ...
            while index < population_size:
                ...
                indiviual = fix_solution(indiviual, acur, int(ref_points[index][0] * dimension))
